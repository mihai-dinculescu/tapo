...
impl PyApiClient {
    #[new]
    pub fn new(tapo_username: String, tapo_password: String, timeout_secs: Option<u8>) -> Result<Self, ErrorWrapper> {
        let client = ApiClient::new(tapo_username, tapo_password, timeout_secs)?;
        Ok(Self { client })
    }
