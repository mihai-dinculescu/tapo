    ...
    while let Some(discovery_result) = discovery.next().await {
        if let Ok(device) = discovery_result {
            match device {
                ...
                DiscoveryResult::Light { device_info, .. } => {
                    info!(
                ...
                DiscoveryResult::Hub { device_info, .. } => {
                    info!(
                        "Found {:?} of model {:?} at IP address {:?}.",
                        device_info.nickname, device_info.model, device_info.ip
                    );
                ...
                _result => {
                    // This is a catch-all for any other device types that may be added in the future.
                    warn!("Found a device that wasn't matched against any known type.")
                }
            }
