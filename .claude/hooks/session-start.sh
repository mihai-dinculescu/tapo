...
if ! command -v gh &>/dev/null; then
  ...
  if [ -z "$GH_VERSION" ]; then
    echo "Warning: Failed to fetch gh version from GitHub API, using fallback" >&2
    GH_VERSION="2.87.3"
  fi
...
if ! command -v cargo-make &>/dev/null; then
  ...
  if [ -z "$CM_VERSION" ]; then
    echo "Warning: Failed to fetch cargo-make version from GitHub API, using fallback" >&2
    CM_VERSION="0.37.24"
  fi
